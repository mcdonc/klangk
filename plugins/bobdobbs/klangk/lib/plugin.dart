...
class BobDobbsPlugin extends ToolPlugin with ChangeNotifier {
  bool _show = false;
  ...
  Widget? buildOverlay(BuildContext context) {
    return _BobDobbsOverlay(plugin: this);
  }
}

class _BobDobbsOverlay extends StatefulWidget {
  final BobDobbsPlugin plugin;
  const _BobDobbsOverlay({required this.plugin});
...
class _BobDobbsOverlayState extends State<_BobDobbsOverlay>
  ...
  void _onUpdate() {
    if (widget.plugin._show && !_controller.isAnimating) {
        ...
        child: Container(
          ...
          child: Center(
            child: AnimatedBuilder(
              ...
              child: Image.asset(
                'assets/bobdobbs.png',
                package: 'klangk_plugin_bobdobbs',
                width: 200,
                height: 200,
              ),
            ),
