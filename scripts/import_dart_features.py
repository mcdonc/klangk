...
# (compiled in but not in defaults). Today three local features ship dormant:
# ``word-count`` (TS-only — compiled into the workspace image, absent from
# the manifest's features[], #1700), ``soliplex`` (#1664, vendored local in
# #1686), and ``boingball`` (#3149: dropped from the default set so the
# shipped CSP can omit script-src 'unsafe-eval'). None surface unless an
# operator opts in via KLANGKD_FEATURES_ENABLE.
DEFAULT_FEATURES = [
    "beep",
    "bobdobbs",
    "browser-fetch",
