#!/usr/bin/env bash
# Create a stub klangk_features package so flutter pub get works without
# running the full import_dart_features.py codegen. Useful for CI and
# first-time checkouts before features are materialized.
#
# This is a *dev-shell/CI convenience* only — the real build
# (flutterbuildweb.sh / build-*-image.sh) materializes features into its own
...
# CI step) restores it.
#
# Usage: bash scripts/stub_dart_features.sh [stub-dir]
...
if [ -e "$FRONTEND_DIR/pubspec_overrides.yaml" ]; then
  # Symlink exists and resolves (non-dangling) — real features (or a prior
...
// Stub — no features. Generated by scripts/stub_dart_features.sh
import 'package:klangk_plugin_api/klangk_plugin_api.dart';
List<ToolPlugin> createAllFeatures() => [];
...
echo "Created stub klangk_features at $STUB_DIR"
