...
/// Pump the feature's app bar icon + overlay inside a minimal app, mirroring
/// how klangk renders them: icon in AppBar.actions, overlay in body Stack.
Future<void> pumpOverlay(WidgetTester tester, SoliplexFeature feature) =>
    tester.pumpWidget(
      MaterialApp(
        home: Builder(
          builder: (context) {
...
SoliplexFeature _feature() => SoliplexFeature(
...
void main() {
  ...
  testWidgets('no-auth server (empty /api/login) shows open, not an error', (
    ...
    await pumpOverlay(tester, feature);
    await tester.pump();
    await tester.tap(find.byKey(_iconKey));
    await tester.pumpAndSettle();
    await tester.tap(find.byKey(const ValueKey('soliplex_connect_default')));
    ...
    expect(feature.authenticated, isTrue);
    expect(
      find.byKey(const ValueKey('soliplex_logout_default')),
      findsOneWidget,
    );
