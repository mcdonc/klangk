# klangk_feature_soliplex

Soliplex knowledge-base feature for [Klangk](https://github.com/mcdonc/klangk),
...
This package is **vendored into the Klangk monorepo** at `features/soliplex/`
...
| Feature-agnostic `BrowserDelegate` / `ws_client.sendBrowserChunk` relay   | klangk `main` (#91)                  | merged |
...
This package supplies the last row. `SoliplexFeature` exposes:
...
```bash
update-features                       # scripts/update_features.py — vendors the repo
python3 scripts/import_dart_features.py   # scans <name>/klangk/, writes createAllFeatures()
cd src/frontend && flutter pub get && flutter build
```

`import_dart_features.py` finds `soliplex/klangk/lib/feature.dart`, sees
`class SoliplexFeature extends ToolPlugin`, and emits it into `createAllFeatures()`.
...
```text
klangk/                           # Flutter package (main's feature convention)
  pubspec.yaml
  lib/
    klangk_feature_soliplex.dart   # barrel: exports SoliplexFeature
    feature.dart                   # ToolPlugin: handlers + streamingHandlers + auth overlay
