...
class BoingBallFeature extends ToolPlugin with ChangeNotifier {
  bool _active = false;
  double _speed = 1.0;
  ...
  Future<void> _loadConfig() async {
    ...
    try {
      ...
      if (resp.statusCode == 200) {
        ...
        final speed = data['boing_speed'] as String?;
...
void _playBoingSound({required double panX, bool isFloor = true}) {
  ...
  final ctx = _audioCtx();
  ctx.resume();
  void play(web.AudioBuffer buf) {
    final src = ctx.createBufferSource();
    src.buffer = buf;
    src.playbackRate.value = rate;
    final g = ctx.createGain();
    g.gain.value = vol;
    final p = ctx.createStereoPanner();
    p.pan.value = pan;
    src.connect(g);
    g.connect(p);
  ...
  final cached = _boingBuf;
  if (cached != null) {
    play(cached);
    return;
  }
  // Fresh copy so the JS view exposes exactly the decoded bytes (no
  // offset/length view into a larger buffer), then FileReader → ArrayBuffer →
  // decodeAudioData (the only public-API path to a JSArrayBuffer).
  final bytes = Uint8List.fromList(base64Decode(_boingMp3B64));
...
class _BoingOverlay extends StatefulWidget {
  final BoingBallFeature feature;
  const _BoingOverlay({required this.feature});
...
class _BoingOverlayState extends State<_BoingOverlay>
  ...
  void _onUpdate() {
    if (!mounted) return;
    if (widget.feature._active) {
      widget.feature._active = false;
      _startAnimation();
    }
  }
  ...
  void _startAnimation() async {
    ...
    _vx = 0.004 * widget.feature._speed;
    _vy = 0.0;
    _spinDir = 1;
    _frame = 0;
  ...
  void _tick() {
    ...
    final speed = widget.feature._speed;
