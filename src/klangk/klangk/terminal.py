        ...
        *agent_home* (e.g. ``/home/clanker``) is the session's HOME. The
        ...
        async with self.registry.get_service_session_lock(container_id):
            ...
            try:
                ...
                # Process-ledger agent anchor (#2520): resolve the
                # service window's pane pid and register it — launches
                # descending from it are attributed to the agent. Best
                # effort: a failed lookup logs and skips (attribution
                # degrades to unknown, never breaks the terminal path).
                try:
