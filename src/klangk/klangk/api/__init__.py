...
``root_router`` for ``/health``, ``/audit``, and ``/empty``.  The few
endpoints that do
...
# ``klangk.api.auth`` (the attribute) now points at the route
# submodule because of the import above; point the bare ``auth`` name back
# at the klangk.auth logic module the instance endpoints use.
auth = _auth_logic
...
async def get_config(
    ...
    config = {
        ...
        # How many previous passwords are retired into history and
        ...
        # reach (#3173); 0 = no constraint. Clients with
