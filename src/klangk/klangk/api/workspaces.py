...
async def add_workspace_member(
    ...
    # Add ACL entries granting the target user view+terminal+files(+download)
    ...
    for perm in ("view", "terminal", "files", "files-download"):
...
async def add_workspace_group(
    ...
    """Share a workspace with a group (view/terminal/files(+download))."""
    ...
    for i, perm in enumerate(["view", "terminal", "files", "files-download"]):
        await app.state.model.acl.add_acl_entry(
            resource,
            max_pos + 1 + i,
            ACTION_ALLOW,
