...
class TestExecDispatch:
    ...
    async def test_dispatch_chat_send(self, user, app_state):
        app_state = _make_app_state()
    ...
    async def test_dispatch_chat_delete(self, user, app_state):
        app_state = _make_app_state()
...
class TestChatLoadMoreDispatch:
    async def test_dispatch_chat_load_more(self, user, app_state):
        app_state = _make_app_state()
...
class TestResetWorkspaceState:
    ...
    async def test_reset_cleans_agent_state(self, app_state):
    ...
    async def test_reset_stops_agent_session(self, app_state):
        """reset_workspace stops the Pi RPC subprocess via agent.stop_session."""
        app_state = _make_app_state()
        sockets = app_state.state.sockets
        ws_id = "ws-agent-stop"
        with patch.object(
            app_state.state.agents, "stop_session", new_callable=AsyncMock
        ) as mock_stop:
            await reset_workspace_state(sockets, ws_id)
...
class TestMentionsAgent:
    async def test_detects_mention(self, agent_user, app_state):
        ...
        assert await mentions_agent("@klangk hello", app_state)
        assert await mentions_agent("hey @klangk what's up", app_state)
        assert await mentions_agent("@KLANGK help", app_state)
    ...
    async def test_no_false_positives(self, agent_user, app_state):
        ...
        assert not await mentions_agent("klangk without at sign", app_state)
        assert not await mentions_agent("@klangky partial match", app_state)

    async def test_follows_agent_handle_rename(self, agent_user, app_state):
        """Detection reads the live agent handle from the DB (not a stale
        cache) — the regression behind #875 stays covered even though the
        agent handle is now fixed (#2718): the test proves the regex is
        rebuilt from the DB row whenever the cache drops.
        """
        from klangk.wshandler import mentions_agent

        # Sanity: fixed handle is detected.
        assert await mentions_agent("@klangk hello", app_state)

        # Directly rewrite the row (out-of-band — the API refuses agent
        # renames now, #2718) and drop the cached user.
        ...
        assert await mentions_agent("@RenamedBot hello", app_state)
        # ... and the stale old handle no longer matches.
        assert not await mentions_agent("@klangk hello", app_state)

        # Restore (keep other tests' cache clean).
        async with app_state.state.db.transaction() as db:
            await db.execute(
                "UPDATE users SET handle = ? WHERE id = ?",
                ("klangk", model.AGENT_USER_ID),
            )
        app_state.state.model.users.clear_agent_cache()
...
class TestAddressesOtherUser:
    async def test_starts_with_other_mention(self, agent_user, app_state):
        ...
        assert await addresses_other_user(
            "@alice@test.com what do you think?", app_state
        )

    async def test_starts_with_agent_mention(self, agent_user, app_state):
        from klangk.wshandler import addresses_other_user

        assert not await addresses_other_user("@klangk hello", app_state)
        assert not await addresses_other_user("@KLANGK help", app_state)
...
class TestChatFollowUp:
    ...
    async def test_interjection_expired(
        self, workspace, user, agent_user, app_state
    ):
...
class TestChatSend:
    ...
    async def test_chat_send_broadcasts(
        self, workspace, user, agent_user, app_state
    ):
        app_state = _make_app_state()
        sockets = app_state.state.sockets
    ...
    async def test_chat_send_with_mention(
        self, workspace, user, agent_user, app_state
    ):
    ...
    async def test_chat_send_requires_chat_perm(self, workspace, user):
        ...
        with (
            patch.object(conn, "_has_perm", new=AsyncMock(return_value=False)),
            patch.object(
                ChatModel, "add_chat_message", new_callable=AsyncMock
            ) as mock_add,
        ):
            await conn.handle_chat_send(
                {"message": "@klangk delete everything"}
            )
    ...
    async def test_chat_send_agent_mention(
        ...
        """@klangk sends thinking event + agent response."""
        app_state = _make_app_state()
        sockets = app_state.state.sockets

        # Seed a prior agent message so context filtering is exercised
        agent_email = await app_state.state.model.users.agent_email()
        ...
        try:
            with patch.object(
                ...
                await conn.handle_chat_send(
                    {"message": "@klangk what time is it?"}
                )
                await _await_agent_run(workspace["id"])
    ...
    async def test_chat_send_agent_mention_empty_prompt(
        ...
        """@klangk with no prompt uses default greeting."""
        app_state = _make_app_state()
        sockets = app_state.state.sockets
        ...
        try:
            with patch.object(
                ...
                await conn.handle_chat_send({"message": "@klangk"})
                await _await_agent_run(workspace["id"])
            calls = [c[0][0] for c in sock.send_json.call_args_list]
            agent_msgs = [
                c
    ...
    async def test_chat_send_agent_error(
        self, workspace, user, agent_user, app_state
    ):
        ...
        try:
            with patch.object(
                ...
                await conn.handle_chat_send({"message": "@klangk help"})
                await _await_agent_run(workspace["id"])
            calls = [c[0][0] for c in sock.send_json.call_args_list]
            agent_msgs = [
                c
    ...
    async def test_chat_send_agent_process_died(
        ...
        try:
            with patch.object(
                ...
                await conn.handle_chat_send({"message": "@klangk hello"})
                await _await_agent_run(workspace["id"])
            calls = [c[0][0] for c in sock.send_json.call_args_list]
            sys_msgs = [
                c
    ...
    async def test_chat_send_no_agent_mention(
        ...
        """Messages without @klangk don't trigger agent response."""
        app_state = _make_app_state()
        sockets = app_state.state.sockets
    ...
    async def test_chat_send_mention_supersedes_prior_run(
        ...
        try:
            with patch(
                ...
                await conn1.handle_chat_send({"message": "@klangk first"})
                ...
                await conn2.handle_chat_send({"message": "@klangk second"})
...
class TestPresence:
    ...
    async def test_presence_join_broadcast(self, user, agent_user, app_state):
    ...
    async def test_presence_leave_broadcast(self, user, app_state):
        ...
        conn1 = _base_conn(user=user, ws=sock1, app_state=app_state)
        other = await app_state.state.model.users.create_user(
            "lv@test.com", "hash", verified=True
        )
        conn2 = _base_conn(
    ...
    async def test_presence_leave_suppressed_on_reconnect(
        self, user, app_state
    ):
        ...
        other = await app_state.state.model.users.create_user(
            "flap@test.com", "hash", verified=True
        )
        await app_state.state.model.acl.add_acl_entry(
            f"/workspaces/{workspace['id']}",
            100,
    ...
    async def test_presence_leave_multi_tab(self, user, app_state):
        ...
        conn2 = _base_conn(user=user, ws=sock2, app_state=app_state)
        other = await app_state.state.model.users.create_user(
            "mt@test.com", "hash", verified=True
        )
        conn3 = _base_conn(
        ...
        async with _conn_in_workspace(user, workspace["id"]) as (
            ...
            with patch.object(
                ...
                users = await get_presence_list(
                    workspace["id"], app_state.state.sockets
                )
...
class TestAgentMentionOtherMsgsContext:
    async def test_other_user_messages_prepended_to_prompt(
        ...
        agent_email = (await app_state.state.model.users.get_agent_user())[
            "email"
        ]
