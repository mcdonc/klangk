...
class TestKillOrphanedPasta:
    """Unit tests for _kill_orphaned_pasta."""

    def setup_method(self):
        app_state = _make_app_state()
        self.registry = app_state.state.container_registry

    async def test_skips_non_linux(self, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        # Should return immediately, no subprocess calls.
        await self.registry._kill_orphaned_pasta({9020})

    async def test_kills_pasta_process(self, monkeypatch, tmp_path):
        monkeypatch.setattr("sys.platform", "linux")
        killed = []

        async def fake_fuser(*args, **kw):
            m = AsyncMock()
            m.communicate = AsyncMock(return_value=(b"12345", b""))
            m.returncode = 0
            return m

        comm_file = tmp_path / "comm"
        comm_file.write_text("passt.avx2\n")

        monkeypatch.setattr(asyncio, "create_subprocess_exec", fake_fuser)
        monkeypatch.setattr(
            "klangk.container.Path",
            lambda p: comm_file if "comm" in str(p) else Path(p),
        )
        monkeypatch.setattr(
    ...
    async def test_skips_non_pasta_process(self, monkeypatch, tmp_path):
        monkeypatch.setattr("sys.platform", "linux")
        killed = []
    ...
    async def test_fuser_not_found(self, monkeypatch):
        monkeypatch.setattr("sys.platform", "linux")

        async def raise_fnf(*args, **kw):
    ...
    async def test_fuser_returns_pids_on_stderr(self, monkeypatch, tmp_path):
        """Some fuser versions output PIDs to stderr."""
        monkeypatch.setattr("sys.platform", "linux")
        killed = []

        async def fake_fuser(*args, **kw):
            m = AsyncMock()
            # PIDs on stderr, not stdout.
            m.communicate = AsyncMock(return_value=(b"", b"  99999"))
            m.returncode = 0
            return m

        comm_file = tmp_path / "comm"
        comm_file.write_text("pasta\n")
    ...
    async def test_fuser_oserror(self, monkeypatch):
        """OSError from fuser is caught and continues to next port."""
        monkeypatch.setattr("sys.platform", "linux")

        async def raise_os(*args, **kw):
            raise OSError("broken pipe")

        monkeypatch.setattr(asyncio, "create_subprocess_exec", raise_os)
        await self.registry._kill_orphaned_pasta({9020, 9021})

    async def test_proc_comm_gone(self, monkeypatch, tmp_path):
        """PID disappears between fuser and /proc read."""
        monkeypatch.setattr("sys.platform", "linux")

        async def fake_fuser(*args, **kw):
            m = AsyncMock()
            m.communicate = AsyncMock(return_value=(b"77777", b""))
            m.returncode = 0
            return m

        monkeypatch.setattr(asyncio, "create_subprocess_exec", fake_fuser)
        # /proc/77777/comm won't exist — Path(...).read_text() raises.
        await self.registry._kill_orphaned_pasta({9020})

    async def test_kill_permission_denied(self, monkeypatch, tmp_path):
        """os.kill fails with PermissionError — logged, not raised."""
        monkeypatch.setattr("sys.platform", "linux")
