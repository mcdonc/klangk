...
allow/reject install OUTPUT ACCEPT/REJECT rules under _LOCK (atomic w.r.t.
sweep_once); drop_for_host revokes a host's rules; the TTL sweeper
(_async_sweeper / sweep_once) reaps expired rules. _learn_all / _record_hosts
/ _host_for drive the learned-IP map the DNS + NFQUEUE paths share.
"""
...
from .state import _LEARNED, _LOCK, _REJECTED
...
def _rule_exists(ip: str, port: int | None) -> bool:
    return (
        subprocess.run(
            [config.IPT, "-C", "OUTPUT", *_rule_args(ip, port)],
            capture_output=True,
        ).returncode
        == 0
    )
...
def _install(ip: str, port: int | None) -> None:
    ...
    if _rule_exists(ip, port):
        return
    subprocess.run(
        [config.IPT, "-I", "OUTPUT", "1", *_rule_args(ip, port)],
        capture_output=True,
    )
...
def _remove(ip: str, port: int | None) -> None:
    ...
    subprocess.run(
        [config.IPT, "-D", "OUTPUT", *_rule_args(ip, port)],
        capture_output=True,
    )
