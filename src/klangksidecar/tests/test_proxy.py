...
class TestParseSpecs:
    ...
    def test_bare_is_exact_dot_is_inclusive_wildcard_is_subdomains(
        ...
        assert proxy.parse_specs() == [
            ("github.com", 443, proxy.EXACT),  # bare -> exact
            ("pypi.org", None, proxy.SUBDOMAINS),  # *.pypi.org
            ("pypi.org", None, proxy.INCLUSIVE),  # .pypi.org
            ("bare.com", 8443, proxy.EXACT),
        ]

    def test_wildcard_with_port(self, proxy, monkeypatch):
        monkeypatch.setenv("KLANGKNETWORK_EGRESS_ALLOW", "*.pypi.org:443")
        assert proxy.parse_specs() == [("pypi.org", 443, proxy.SUBDOMAINS)]

    def test_inclusive_with_port(self, proxy, monkeypatch):
        monkeypatch.setenv("KLANGKNETWORK_EGRESS_ALLOW", ".pypi.org:443")
        assert proxy.parse_specs() == [("pypi.org", 443, proxy.INCLUSIVE)]
...
class TestPortsFor:
    ...
    def test_bare_host_is_exact_apex_only(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(
            proxy.allowlist, "SPECS", [("github.com", None, proxy.EXACT)]
        )
    ...
    def test_inclusive_matches_apex_and_subdomains(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(
            proxy.allowlist, "SPECS", [("github.com", None, proxy.INCLUSIVE)]
        )
    ...
    def test_wildcard_excludes_apex(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(
            proxy.allowlist, "SPECS", [("pypi.org", None, proxy.SUBDOMAINS)]
        )
    ...
    def test_wildcard_with_port(self, proxy, monkeypatch):
        monkeypatch.setattr(
            proxy.allowlist, "SPECS", [("pypi.org", 443, proxy.SUBDOMAINS)]
        )
        assert proxy.ports_for("pypi.org") == set()
    ...
    def test_port_scope_exact(self, proxy, monkeypatch):
        monkeypatch.setattr(
            proxy.allowlist, "SPECS", [("github.com", 443, proxy.EXACT)]
        )
    ...
    def test_all_ports_spec_dominates(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "SPECS",
            [
                ("github.com", None, proxy.EXACT),
                ("github.com", 443, proxy.EXACT),
            ],
        )
        assert proxy.ports_for("github.com") is None
    ...
    def test_session_host_allows_consulted_alongside_specs(self, proxy, monkeypatch):
        ...
        # SESSION_HOST_ALLOWS (#2372, #2377, #2434): ports_for treats that exact
        ...
        monkeypatch.setattr(
            proxy.state,
            "SESSION_HOST_ALLOWS",
            [("example.com", 443, proxy.EXACT, float("inf"))],
        )
    ...
    def test_add_session_host_dedups_and_refreshes(self, proxy, monkeypatch):
        ...
        proxy.state.SESSION_HOST_ALLOWS.clear()
        proxy.add_session_host("example.com", 443, 300)  # expire 1300
        proxy.add_session_host("example.com", 443, 60)  # dup -> max(1300, 1060)
        proxy.add_session_host("example.com", 8443, 300)  # different port
        assert proxy.state.SESSION_HOST_ALLOWS == [
            ("example.com", 443, proxy.EXACT, 1300.0),
            ("example.com", 8443, proxy.EXACT, 1300.0),
        ]
    ...
    def test_session_host_allows_ttl(self, proxy, monkeypatch):
        ...
        # ports_for via the shared host_matches.
        monkeypatch.setattr(
            proxy.state,
            "SESSION_HOST_ALLOWS",
            [("example.com", 443, proxy.EXACT, float("inf"))],
        )
        assert proxy.session_host_allows_ttl("example.com", 443)
        assert proxy.session_host_allows_ttl("api.example.com", 443) is None
        assert proxy.session_host_allows_ttl("example.com", 80) is None
        assert proxy.session_host_allows_ttl("other.com", 443) is None
        assert proxy.session_host_allows_ttl("evilexample.com", 443) is None
        assert proxy.session_host_allows_ttl("", 443) is None
        # all-ports entry (p is None) matches any port -- defensive (entries are
        # added port-scoped, but the predicate tolerates None).
        monkeypatch.setattr(
            proxy.state,
            "SESSION_HOST_ALLOWS",
            [("example.com", None, proxy.EXACT, float("inf"))],
        )
        assert proxy.session_host_allows_ttl("example.com", 8080)
    ...
    def test_session_allow_rule_cap_returns_remaining(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(
            proxy.state,
            "SESSION_HOST_ALLOWS",
            [("example.com", 443, proxy.EXACT, 1005.0)],  # 5s remaining
        )
        assert proxy.session_allow_rule_cap("example.com") == 5.0
        assert (
            proxy.session_allow_rule_cap("api.example.com") is None
        )  # exact -> no subdomain
        assert proxy.session_allow_rule_cap("other.com") is None
    ...
    def test_session_allow_rule_cap_none_for_static_spec(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(
            proxy.allowlist, "SPECS", [("example.com", None, proxy.EXACT)]
        )
        monkeypatch.setattr(proxy.time, "time", lambda: 1000.0)
        monkeypatch.setattr(
            proxy.state,
            "SESSION_HOST_ALLOWS",
            [("example.com", 443, proxy.EXACT, 1005.0)],
        )
        assert proxy.session_allow_rule_cap("example.com") is None
    ...
    def test_session_allow_rule_cap_expired_entry_pruned(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(
            proxy.state,
            "SESSION_HOST_ALLOWS",
            [("example.com", 443, proxy.EXACT, 999.0)],  # already expired
        )
        assert proxy.session_allow_rule_cap("example.com") is None
        assert proxy.state.SESSION_HOST_ALLOWS == []  # pruned
    ...
    def test_session_allow_rule_cap_min_across_entries(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            "SESSION_HOST_ALLOWS",
            [
                ("example.com", 443, proxy.EXACT, 1005.0),  # 5s
                ("example.com", 8443, proxy.EXACT, 1300.0),  # 300s
            ],
        )
        assert proxy.session_allow_rule_cap("example.com") == 5.0
    ...
    def test_add_session_deny_dedups_and_refreshes(self, proxy, monkeypatch):
        ...
        proxy.add_session_deny("example.com", 443, 300)  # expire 1300
        proxy.add_session_deny("example.com", 443, 60)  # dup -> max(1300, 1060)
        proxy.add_session_deny("example.com", 8443, 300)  # different port
    ...
    def test_session_host_denies_ttl(self, proxy, monkeypatch):
        ...
        # test_session_host_allows_ttl via the shared host_matches.
        monkeypatch.setattr(
            proxy.state,
            "SESSION_HOST_DENIES",
            [("example.com", 443, proxy.EXACT, float("inf"))],
        )
        assert proxy.session_host_denies_ttl("example.com", 443)
        assert proxy.session_host_denies_ttl("api.example.com", 443) is None
        assert proxy.session_host_denies_ttl("example.com", 80) is None
        assert proxy.session_host_denies_ttl("other.com", 443) is None
        assert proxy.session_host_denies_ttl("evilexample.com", 443) is None
        assert proxy.session_host_denies_ttl("", 443) is None
        ...
        assert proxy.session_host_denies_ttl("example.com", 8080)
...
class TestRuleArgs:
    ...
    def test_all_ports(self, proxy):
        assert proxy.rule_args("1.2.3.4", None) == [
            "-d",
            "1.2.3.4",
            "-j",
            "ACCEPT",
    ...
    def test_scoped_port(self, proxy):
        assert proxy.rule_args("1.2.3.4", 443) == [
            "-d",
            "1.2.3.4",
            "-p",
            "tcp",
...
class TestRuleArgsInvariant:
    ...
    -D (remove) build IDENTICAL args from one ``rule_args``. A future edit
    ...
    def test_check_install_remove_share_args(self, proxy, monkeypatch, port):
        ...
        proxy.install("1.2.3.4", port)
        proxy.rule_exists("1.2.3.4", port)
        proxy.remove("1.2.3.4", port)
        expected = proxy.rule_args("1.2.3.4", port)
        ops = set()
        for cmd in calls:
...
class TestDecision:
    ...
    def test_empty_qname_denies(self, proxy):
        assert proxy.decision("", {443}) == (True, set())

    def test_none_ports_allows_all_ports(self, proxy):
        assert proxy.decision("a.com", None) == (False, {None})

    def test_empty_set_denies(self, proxy):
        assert proxy.decision("a.com", set()) == (True, set())

    def test_port_set_scoped_allow(self, proxy):
        assert proxy.decision("a.com", {443, 8443}) == (
            False,
            {443, 8443},
        )
...
class TestTTLAndSweep:
    ...
    def test_min_ttl_floors_zero_ttl(self, learned, monkeypatch):
        ...
        assert learned.LEARNED["1.2.3.4"]["expire"] == 0.0 + learned.MIN_TTL
    ...
    def test_dedup_skips_install_when_rule_exists(self, learned, monkeypatch):
        ...
        assert learned.LEARNED["1.2.3.4"]["ports"] == {443}
    ...
    def test_refresh_extends_expire_only_forward(self, learned, monkeypatch):
        ...
        assert learned.LEARNED["1.2.3.4"]["expire"] == 2500.0
    ...
    def test_all_ports_rule_uses_none_port(self, learned, monkeypatch):
        ...
        assert learned.LEARNED["1.2.3.4"]["ports"] == {None}
    ...
    def test_cross_domain_shared_ip_accumulates_ports(self, learned, monkeypatch):
        ...
        assert learned.LEARNED["9.9.9.9"]["ports"] == {443, 8443}

    def test_sweep_removes_expired_rules(self, learned, monkeypatch):
        removed = []
        monkeypatch.setattr(
            learned.rules, "remove", lambda ip, port: removed.append((ip, port))
        )
        learned.LEARNED["1.2.3.4"] = {"expire": 500.0, "ports": {443, None}}
        ...
        assert "1.2.3.4" not in learned.LEARNED

    def test_sweep_keeps_live_ips(self, learned, monkeypatch):
        monkeypatch.setattr(learned.rules, "remove", lambda *a: None)
        learned.LEARNED["1.2.3.4"] = {"expire": 2000.0, "ports": {443}}
        assert learned.sweep_once(now=1000.0) == []
        assert "1.2.3.4" in learned.LEARNED
    ...
    def test_sweep_boundary_keeps_equal_expire(self, learned, monkeypatch):
        ...
        monkeypatch.setattr(learned.rules, "remove", lambda *a: None)
        learned.LEARNED["dead"] = {"expire": 1000.0, "ports": {443}}
        learned.LEARNED["live"] = {"expire": 1000.001, "ports": {443}}
        gone = learned.sweep_once(now=1000.0)
        assert [ip for ip, _ in gone] == ["dead"]
        assert "live" in learned.LEARNED
    ...
    def test_consent_allow_rule_expire_is_separate_from_dns_ttl(
        ...
        # the host-mapping DNS TTL. record_hosts records the IP with a long DNS
        ...
        monkeypatch.setattr(learned.time, "time", lambda: 0.0)
        learned.record_hosts([("1.2.3.4", 300)], "evil.test")  # DNS TTL 300s
        learned.allow(
    ...
    def test_sweep_removes_consent_rule_at_verdict_keeps_host_mapping(
        ...
        # valid -- so host_for can still name the host for the fresh
        # re-prompt. Only after the DNS TTL does the record itself go.
        removed = []
        monkeypatch.setattr(
            learned.rules, "remove", lambda ip, port: removed.append((ip, port))
        )
        learned.LEARNED["1.2.3.4"] = {
    ...
    def test_consent_allow_rule_not_extended_by_re_resolve_regression(
        ...
        # longer DNS TTL). allow(short) -> record_hosts(long) -> sweep(at
        ...
        verdict = learned.LEARNED["1.2.3.4"]["rule_expire"]
        # the re-resolve (long DNS TTL) must NOT extend the rule's lifetime.
        learned.record_hosts([("1.2.3.4", 300)], "evil.test")
        rec = learned.LEARNED["1.2.3.4"]
    ...
    def test_timed_allow_dns_learn_lapses_at_verdict_under_default_min_ttl(
        ...
        # LEARNED (no mock). A mid-window re-resolve carrying a long DNS TTL
        ...
        monkeypatch.setattr(learned.time, "time", lambda: 0.0)
        learned.record_hosts([("1.2.3.4", 600)], "example.com")
        learned.allow("1.2.3.4", 443, 300, floor=False)  # 5m verdict
        assert learned.LEARNED["1.2.3.4"]["rule_expire"] == 300.0
        ...
        learned.learn_all([("1.2.3.4", 600)], {443}, cap=180)
        rec = learned.LEARNED["1.2.3.4"]
    ...
    def test_short_verdict_lapses_at_verdict_not_min_ttl(self, learned, monkeypatch):
        ...
        assert learned.LEARNED["1.2.3.4"]["rule_expire"] == 5.0  # NOT 30
        ...
        learned.learn_all([("1.2.3.4", 300)], {443}, cap=3)
        assert learned.LEARNED["1.2.3.4"]["rule_expire"] == 5.0  # still 5
        # sweep at the verdict (t=5) -> rule gone; host mapping retained.
        gone = learned.sweep_once(now=5.0)
        assert gone == [("1.2.3.4", {443})]
        assert removed == [("1.2.3.4", 443)]
        assert learned.LEARNED["1.2.3.4"]["host"] == "example.com"
    ...
    def test_dns_learn_uncapped_extends_past_verdict(self, learned, monkeypatch):
        ...
        # re-prompt). Here learn_all is called WITHOUT the cap (as if the
        # session_allow_rule_cap step were missing); the realistic test above
        ...
        monkeypatch.setattr(learned.time, "time", lambda: 120.0)
        learned.learn_all([("1.2.3.4", 600)], {443})  # NO cap -> DNS TTL wins
        assert (
            learned.LEARNED["1.2.3.4"]["rule_expire"] == 720.0
    ...
    def test_static_spec_learn_0_ttl_floored_at_min_ttl(self, learned, monkeypatch):
        ...
        # real allow() -> LEARNED (the floor is applied inside allow, so a mock
        ...
        learned.learn_all([("1.2.3.4", 0)], {443})
        rec = learned.LEARNED["1.2.3.4"]
    ...
    def test_reject_installs_reject_rule_and_records(self, learned, monkeypatch):
        ...
        assert learned.REJECTED[("1.2.3.4", 80, 0)] == 1010.0
    ...
    def test_reject_connection_scoped_adds_sport(self, learned, monkeypatch):
        ...
        # key keeps the two rule kinds distinct in REJECTED.
        runs = []
        ...
        assert learned.REJECTED[("1.2.3.4", 443, 50000)] == 1010.0
    ...
    def test_sweep_removes_expired_reject_rules(self, learned, monkeypatch):
        ...
        monkeypatch.setattr(learned.subprocess, "run", fake_run)
        learned.LEARNED.clear()
        learned.REJECTED.clear()
        learned.REJECTED[("1.2.3.4", 80, 0)] = 100.0  # expired
        learned.REJECTED[("5.6.7.8", 443, 0)] = 9999.0  # live
        learned.sweep_once(now=500.0)
        assert any(
            "1.2.3.4" in a and "REJECT" in a and "tcp-reset" in a for a in removed
        )
        assert ("1.2.3.4", 80, 0) not in learned.REJECTED
        assert ("5.6.7.8", 443, 0) in learned.REJECTED  # live one kept
    ...
    def test_all_ports_allow_supersedes_prior_port_denies(self, learned, monkeypatch):
        ...
        monkeypatch.setattr(learned.rules, "install", lambda *a: None)
        learned.REJECTED.clear()
        learned.REJECTED[("1.2.3.4", 443, 0)] = 9999.0
        learned.REJECTED[("1.2.3.4", 80, 0)] = 9999.0
        learned.REJECTED[("5.6.7.8", 443, 0)] = 9999.0  # different IP, untouched
        ...
        assert ("1.2.3.4", 443, 0) not in learned.REJECTED
        assert ("5.6.7.8", 443, 0) in learned.REJECTED
...
class TestRespondAllowedSwallowsFailures:
    ...
    async def test_allow_failure_does_not_propagate(self, proxy, monkeypatch):
        ...
        # respond_allowed swallows).
        await proxy.respond_allowed(
    ...
    async def test_happy_path_allows_each_ip_each_port_and_sends(
        ...
        s = MagicMock()
        await proxy.respond_allowed(s, b"resp", ("127.0.0.1", 1234), "q", {443, 8443})
    ...
    async def test_respond_allowed_no_cap_when_no_session_allow(
        ...
        # production ports_for would have denied this name, so respond_allowed
        # would not run; the cap helper still returns None defensively.)
        monkeypatch.setattr(proxy.allowlist, "SPECS", [])
        monkeypatch.setattr(proxy.state, "SESSION_HOST_ALLOWS", [])
        monkeypatch.setattr(
            proxy.resolve,
            "a_records_with_ttl",
            lambda wire: [("1.2.3.4", 300)],
    ...
    def test_learn_all_cap_bounds_each_rule_ttl(self, proxy, monkeypatch):
        # learn_all threads the cap through to allow() as min(dns_ttl, cap),
        # per IP/port. The host mapping (set separately by record_hosts at the
        ...
        proxy.learn_all([("1.2.3.4", 300), ("5.6.7.8", 60)], {443, 8443}, cap=5)
    ...
    def test_learn_all_no_cap_uses_dns_ttl(self, proxy, monkeypatch):
        ...
        proxy.learn_all([("1.2.3.4", 300)], {443})
...
class TestConsentForward:
    ...
    def test_build_rst_packet_layout_and_checksum(self, proxy):
        ...
        assert proxy.ones_checksum(pseudo + tcp_zero) == cksum
    ...
    # --- ws_url: derive the WS URL from the consent HTTP URL (#2311) ---
    ...
    def test_ws_url_https_to_wss(self, proxy):
        assert (
            proxy.ws_url("https://klangkd.example/ev")
            == "wss://klangkd.example/ws/egress-sidecar"
        )

    def test_ws_url_passthrough_ws(self, proxy):
        # an already-ws(s):// URL is used verbatim
        assert proxy.ws_url("ws://x/ws/egress-sidecar") == "ws://x/ws/egress-sidecar"

    # --- record_hosts / host_for: IP->host map for the SYN consent gate (#2324) ---

    def test_record_hosts_records_ip_to_host_without_accept(self, proxy):
        proxy.LEARNED.clear()
        proxy.record_hosts([("1.2.3.4", 60)], "evil.test")
        rec = proxy.LEARNED["1.2.3.4"]
    ...
    def test_record_hosts_does_not_touch_consent_rule_expire(self, learned):
        # #2408: record_hosts refreshes only the host-mapping expire -- never
    ...
    def test_record_hosts_extends_ttl_for_pure_host_mapping(self, learned):
        ...
        learned.record_hosts([("1.2.3.4", 300)], "b.test")
        rec = learned.LEARNED["1.2.3.4"]
    ...
    def test_host_for_returns_host_when_recorded(self, proxy):
        proxy.LEARNED.clear()
        proxy.record_hosts([("1.2.3.4", 60)], "evil.test")
        assert proxy.host_for("1.2.3.4") == "evil.test"

    def test_host_for_falls_back_to_ip_for_direct_connect(self, proxy):
        proxy.LEARNED.clear()
        assert proxy.host_for("5.6.7.8") == "5.6.7.8"  # no DNS record -> IP
    ...
    async def test_fail_close_pending_resolves_deny_and_clears_cache(
        ...
        proxy.VERDICT_CACHE.clear()
        proxy.VERDICT_CACHE[("1.2.3.4", 443)] = ("allow", 9999999.0)
        c._fail_close_pending()
        assert fut.result() == ("deny", "once")
        assert c._pending == {}
        assert proxy.VERDICT_CACHE == {}  # stale verdicts dropped on disconnect
    ...
    # --- respond_recorded: resolve + record IP->host + respond (no ACCEPT) (#2324) ---
    ...
    async def test_respond_recorded_records_host_and_sends(self, proxy, monkeypatch):
        ...
        s = MagicMock()
        await proxy.respond_recorded(s, b"resp", ("1.2.3.4", 53), "evil.test")
    ...
    async def test_respond_recorded_swallows_sendto_failure(self, proxy, monkeypatch):
        proxy.LEARNED.clear()
        monkeypatch.setattr(proxy.rules, "record_hosts", lambda recs, host: None)
        monkeypatch.setattr(
            proxy.resolve, "a_records_with_ttl", lambda wire: [("1.2.3.4", 60)]
        )
        s = MagicMock()
        s.sendto.side_effect = OSError("gone")
        await proxy.respond_recorded(  # must not raise
    ...
    def test_setup_nfq_consumer_noop_without_netfilterqueue(
        ...
        proxy.setup_nfq_consumer(None)
        assert "netfilterqueue not installed" in capsys.readouterr().out


# --- helpers for the NFQUEUE callback + handle_packet tests (#2311 half B) ---
...
class TestBumpActivity:
    ...
    async def test_sends_again_after_gate_window_elapses(
        ...
        monkeypatch.setattr(proxy.consent, "jittered_gate", lambda: base)
    ...
    async def test_gate_jittered_below_base_lets_early_send_through(
        ...
        monkeypatch.setattr(proxy.consent, "jittered_gate", lambda: base * 0.5)
    ...
    async def test_send_error_is_swallowed(self, proxy, tmp_path):
        ...
        await self.drain(c)  # the scheduled send raises internally, swallowed
...
class TestEgressAcct:
    ...
    def test_acct_match_unmarked_with_exclude(self, proxy):
        m = proxy.rules.acct_match("10.0.0.1")
        ...
        assert proxy.rules.ACCT_COMMENT in m
        assert m[-1] == "ACCEPT"

    def test_acct_match_omits_exclude_when_none(self, proxy):
        m = proxy.rules.acct_match(None)
...
class TestActivitySampler:
    ...
    Per-tick logic is factored into activity_delta for deterministic tests."""

    def test_delta_positive_bumps(self, proxy):
        bumped, prev = proxy.consent.activity_delta(lambda: 1000, 0)
        assert bumped and prev == 1000

    def test_delta_zero_no_bump(self, proxy):
        bumped, prev = proxy.consent.activity_delta(lambda: 500, 500)
    ...
    def test_delta_reset_rebaselines_without_bump(self, proxy):
        ...
        bumped, prev = proxy.consent.activity_delta(lambda: 5, 1000)
    ...
    def test_safe_bytes_read_failure_is_zero(self, proxy):
        ...
        assert proxy.consent.safe_bytes(boom) == 0

    def test_safe_bytes_non_int_is_zero(self, proxy):
        assert proxy.consent.safe_bytes(lambda: "nonsense") == 0
    ...
    async def test_sampler_bumps_only_on_real_traffic(self, proxy):
        ...
        # StopIteration -> safe_bytes -> 0 -> flat. Bumps must total 2.
        ...
        task = asyncio.create_task(
            proxy.consent.activity_sampler(C(), lambda: next(readings), 0.0)
        )
...
class TestNfqueueCallback:
    """The SYN consent gate (#2324, #2329): the non-blocking NFQUEUE ``cb``
    names the host, hands the verdict wait to a task, learns on allow, REJECTs
    on deny, and reuses the verdict for retransmits. ``cb`` +
    ``decide_and_verdict`` are driven directly (the loop-native get_fd /
    ...
    async def decide(self, proxy, pkt, client):
        ...
        proxy.cb(pkt, client)
        if proxy.BG_TASKS:
            await asyncio.gather(*proxy.BG_TASKS)
    ...
    async def test_allow_verdict_accepts_and_learns_ip(self, proxy, monkeypatch):
        ...
        assert learned == [
            ("1.2.3.4", None, proxy.DURATION_FOREVER)
        ]  # learned all-ports
    ...
    async def test_forever_allow_adds_host_to_session_allowlist(
        ...
        # sidecar adds it to SESSION_HOST_ALLOWS so ports_for treats it as
    ...
    async def test_cb_auto_allows_syn_to_session_allowed_host_ip(
        ...
        self._bind(proxy, monkeypatch, client)
        proxy.add_session_host("example.com", 443, proxy.DURATION_FOREVER)
        proxy.record_hosts([("2.2.2.2", 60)], "example.com")
        pkt = _FakePkt(_ip_payload("2.2.2.2", 443))
        proxy.cb(pkt, client)  # sync: auto-allows inline, no task
        await asyncio.sleep(0.1)  # let the executor run allow()
        assert pkt.verdict == "accept"
        client.request.assert_not_awaited()  # no consent prompt
        assert not proxy.BG_TASKS  # no verdict task spawned
        ...
        assert allowed[0][2] >= proxy.DURATION_FOREVER - 5
        # Verdict cached so a retransmit reuses it without re-prompting.
        assert any(v[0] == "allow" for v in proxy.VERDICT_CACHE.values())
    ...
    async def test_cb_does_not_auto_allow_wrong_port(self, proxy, monkeypatch):
        ...
        # exercised through cb, not just the predicate).
        client = MagicMock()
        client.connected = True
        ...
        proxy.add_session_host("example.com", 443, proxy.DURATION_FOREVER)
        proxy.record_hosts([("2.2.2.2", 60)], "example.com")
        proxy.cb(_FakePkt(_ip_payload("2.2.2.2", 80)), client)
        await asyncio.gather(*proxy.BG_TASKS)  # the prompt task ran
    ...
    async def test_timed_allow_is_host_scoped(self, proxy, monkeypatch):
        ...
        # verdict adds the host to SESSION_HOST_ALLOWS, so a CDN-rotated IP of
    ...
    def test_timed_session_allow_expires(self, proxy, monkeypatch):
        ...
        proxy.state.SESSION_HOST_ALLOWS.clear()
        proxy.add_session_host("example.com", 443, 300)  # expire 1300
        assert proxy.session_host_allows_ttl("example.com", 443) == 300.0
        clock[0] = 1400.0  # past the 300s window
        assert proxy.session_host_allows_ttl("example.com", 443) is None
        monkeypatch.setattr(proxy.allowlist, "SPECS", [])
    ...
    async def test_timed_deny_is_host_scoped(self, proxy, monkeypatch):
        ...
        # The verdict adds the host to SESSION_HOST_DENIES, so a CDN-rotated IP
        # of the host -- resolved AFTER the deny, with no per-IP REJECTED rule
    ...
    async def test_cb_auto_denies_syn_to_session_denied_host_ip(
        ...
        self._bind(proxy, monkeypatch, client)
        proxy.add_session_deny("example.com", 443, proxy.DURATION_FOREVER)
        proxy.record_hosts([("2.2.2.2", 60)], "example.com")
        pkt = _FakePkt(_ip_payload("2.2.2.2", 443))
        proxy.cb(pkt, client)  # sync: auto-denies inline, no task
        ...
        assert rejected[0][2] >= proxy.DURATION_FOREVER - 5
        # Verdict cached so a retransmit reuses it.
        assert any(v[0] == "deny" for v in proxy.VERDICT_CACHE.values())
    ...
    async def test_once_deny_does_not_add_session_denylist(self, proxy, monkeypatch):
        ...
        # nothing to SESSION_HOST_DENIES (mirror of once-allow). Timed/forever
    ...
    def test_timed_session_deny_expires(self, proxy, monkeypatch):
        ...
        proxy.state.SESSION_HOST_DENIES.clear()
        proxy.add_session_deny("example.com", 443, 300)  # expire 1300
        assert proxy.session_host_denies_ttl("example.com", 443) == 300.0
        clock[0] = 1400.0  # past the 300s window
        assert proxy.session_host_denies_ttl("example.com", 443) is None
        assert proxy.state.SESSION_HOST_DENIES == []
    ...
    async def test_allow_overrides_in_effect_deny_at_gate(self, proxy, monkeypatch):
        ...
        # at the gate -- cb consults session_host_allows_ttl BEFORE
        # session_host_denies_ttl, so a SYN to a host that is both allowed and
    ...
    async def test_verdict_timeout_drops(self, proxy, monkeypatch):
        ...
        # raises -> decide_and_verdict fail-closes to deny -> drop.
        client = MagicMock()
        client.connected = True
    ...
    async def test_ws_down_fails_fast_not_hang(self, proxy, monkeypatch):
        ...
        rst = []
        monkeypatch.setattr(
            proxy.packets, "send_rst", lambda payload: rst.append(payload)
        )
        self._bind(proxy, monkeypatch, client)
        pkt = _FakePkt(_ip_payload("1.2.3.4", 443))
        proxy.cb(pkt, client)  # cb fails fast inline (no _BG_TASK)
    ...
    async def test_distinct_flows_held_concurrently_not_serialized(
        ...
        # flight), not one-behind-the-other. The pre-fix blocking cb serialized
        ...
        monkeypatch.setattr(proxy.rules, "allow", lambda *a: None)
        self._bind(proxy, monkeypatch, client)
        proxy.cb(_FakePkt(_ip_payload("1.2.3.4", 443)), client)
        proxy.cb(_FakePkt(_ip_payload("5.6.7.8", 80)), client)
        # both verdict tasks started concurrently before either resolved
        for _ in range(100):
            if len(started) == 2:
                break
            await asyncio.sleep(0.001)
        assert len(proxy.BG_TASKS) == 2
    ...
    def test_send_rst_paths_and_debug(self, proxy, monkeypatch, capsys):
        ...
        # back (covers rst_debug's print branch).
        monkeypatch.setattr(proxy.packets, "RST_SOCK", None)
        monkeypatch.setattr(proxy.config, "RST_DEBUG", True)
        proxy.send_rst(syn)
        ...
        proxy.send_rst(b"\x00\x00\x00\x00")  # too short / not IPv4
        ...
        # branch of rst_debug's `if RST_DEBUG`).
        monkeypatch.setattr(proxy.config, "RST_DEBUG", False)
        proxy.send_rst(syn)
    ...
    async def test_retransmit_during_hold_does_not_re_prompt(self, proxy, monkeypatch):
        ...
        # task resolves the flow. Without the INFLIGHT dedup, each retransmit
        ...
        proxy.cb(pkt1, client)  # spawns the task; connection is now in-flight
        proxy.cb(pkt2, client)  # in-flight -> drop, no new task
        assert pkt2.verdict == "drop"  # the retransmit is dropped
        assert len(proxy.BG_TASKS) == 1  # only ONE verdict task
        assert flow in proxy.INFLIGHT  # still held (connection-level key)
        gate.set()  # release the verdict
        await asyncio.gather(*proxy.BG_TASKS)
        assert len(started) == 1  # only ONE request to the decider
        assert flow not in proxy.INFLIGHT  # cleared after verdict
    ...
    async def test_deny_once_re_prompts_new_connection_same_host(
        ...
        # SESSION_HOST_DENIES entry. This is the deny-side mirror of
        ...
        await asyncio.gather(*proxy.BG_TASKS)
        assert proxy.state.SESSION_HOST_DENIES == []  # `once` adds no host-deny
...
def test_duration_ttl_mapping(proxy):
    assert proxy.duration_ttl("once") is None
    assert proxy.duration_ttl("5m") == 300
    assert proxy.duration_ttl("1h") == 3600
    assert proxy.duration_ttl("1w") == 604800
    assert proxy.duration_ttl("tilrestart") == proxy.DURATION_FOREVER
    assert proxy.duration_ttl("forever") == proxy.DURATION_FOREVER
    assert proxy.duration_ttl("bogus") is None  # unknown -> None
...
class TestHandlePacket:
    ...
    async def test_allowed_name_forwards_and_learns(self, proxy, monkeypatch):
        ...
        fwd = AsyncMock()
        monkeypatch.setattr(proxy.resolve, "forward_and_learn", fwd)
        s = MagicMock()
    ...
    async def test_rejected_name_sends_nxdomain_static(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(proxy.resolve, "query_name", lambda wire: "evil.test")
        monkeypatch.setattr(
            proxy.allowlist, "REJECT_SPECS", [("evil.test", None, proxy.EXACT)]
        )
        monkeypatch.setattr(proxy.resolve, "nxdomain_for", lambda d: b"NXD")
        fwd = AsyncMock()
        monkeypatch.setattr(proxy.resolve, "forward_and_learn", fwd)
    ...
    async def test_reject_only_static_denies_all(self, proxy, monkeypatch):
        ...
        # phased out. ports_for on empty SPECS -> set() -> decision denies.
    ...
    async def test_rejected_takes_precedence_over_allowed(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(
            proxy.allowlist, "SPECS", [("dual.test", None, proxy.EXACT)]
        )
        monkeypatch.setattr(
            proxy.allowlist, "REJECT_SPECS", [("dual.test", None, proxy.EXACT)]
        )
        monkeypatch.setattr(proxy.resolve, "nxdomain_for", lambda d: b"NXD")
        fwd = AsyncMock()
        monkeypatch.setattr(proxy.resolve, "forward_and_learn", fwd)
...
class TestDropForHost:
    ...
    def test_allowed_removes_learned_accept(self, proxy, monkeypatch):
        ...
        proxy.LEARNED["1.2.3.4"] = {
            "expire": 9999.0,
            "ports": {443, None},
            "host": "evil.test",
        }
        proxy.drop_for_host("evil.test", "allowed")
        assert "1.2.3.4" not in proxy.LEARNED
    ...
    def test_allowed_direct_ip_removes_learned(self, proxy, monkeypatch):
        ...
        assert proxy.LEARNED["203.0.113.9"]["host"] is None
        proxy.drop_for_host("203.0.113.9", "allowed")
        assert "203.0.113.9" not in proxy.LEARNED  # rule + record removed
    ...
    def test_denied_removes_rejects(self, proxy, monkeypatch):
        ...
        proxy.LEARNED["5.6.7.8"] = {
            "expire": 9999.0,
            "ports": set(),
            "host": "bad.test",
        }
        proxy.REJECTED[("5.6.7.8", 443)] = 9999.0
        proxy.drop_for_host("bad.test", "denied")
        assert ("5.6.7.8", 443) not in proxy.REJECTED
    ...
    def test_denied_direct_ip_match(self, proxy, monkeypatch):
        ...
        proxy.REJECTED[("9.9.9.9", 80)] = 9999.0
        proxy.drop_for_host("9.9.9.9", "denied")
        assert ("9.9.9.9", 80) not in proxy.REJECTED
    ...
    def test_no_match_is_noop(self, proxy, monkeypatch):
        ...
        proxy.drop_for_host("evil.test", "allowed")
        assert "1.2.3.4" in proxy.LEARNED  # untouched
        assert ran == []
    ...
    def test_unknown_decision_is_noop(self, proxy, monkeypatch):
        ...
        assert "1.2.3.4" in proxy.LEARNED
        assert ran == []
    ...
    def test_drop_for_host_returns_candidate_ips(self, proxy, monkeypatch):
        ...
        # clear_verdict_cache can clear VERDICT_CACHE for those IPs.
        proxy.LEARNED.clear()
        proxy.REJECTED.clear()
        monkeypatch.setattr(
            proxy.subprocess,
    ...
    def test_drop_session_denies_removes_host_entries(self, proxy):
        # A denied revoke drops the host's SESSION_HOST_DENIES coverage
        # (#2446); other hosts are left intact. Mirror of the allow revoke
        # (test_drop_session_hosts_removes_host_entries).
        proxy.state.SESSION_HOST_DENIES.clear()
        proxy.state.SESSION_HOST_DENIES[:] = [
            ("evil.test", 443, proxy.EXACT, float("inf")),
            ("other.test", 80, proxy.EXACT, float("inf")),
        ]
        proxy.drop_session_denies("Evil.TEST")  # case-insensitive
        assert proxy.state.SESSION_HOST_DENIES == [
            ("other.test", 80, proxy.EXACT, float("inf"))
        ]

    def test_drop_session_hosts_removes_host_entries(self, proxy):
        # An allowed revoke drops the host's SESSION_HOST_ALLOWS coverage
        # (in-session allow from #2372/#2434); other hosts are left intact.
        proxy.state.SESSION_HOST_ALLOWS.clear()
        proxy.state.SESSION_HOST_ALLOWS[:] = [
            ("evil.test", 443, proxy.EXACT, float("inf")),
            ("other.test", 80, proxy.EXACT, float("inf")),
        ]
        proxy.drop_session_hosts("Evil.TEST")  # case-insensitive
        assert proxy.state.SESSION_HOST_ALLOWS == [
            ("other.test", 80, proxy.EXACT, float("inf"))
        ]
    ...
    def test_clear_verdict_cache_drops_host_entries(self, proxy):
        ...
        proxy.VERDICT_CACHE.clear()
        proxy.VERDICT_CACHE[("10.0.0.1", 1, "1.2.3.4", 443)] = ("allow", 9e9)
        proxy.VERDICT_CACHE[("10.0.0.1", 2, "5.6.7.8", 443)] = ("deny", 9e9)
        proxy.clear_verdict_cache({"1.2.3.4", "evil.test"})
        assert ("10.0.0.1", 1, "1.2.3.4", 443) not in proxy.VERDICT_CACHE
        assert ("10.0.0.1", 2, "5.6.7.8", 443) in proxy.VERDICT_CACHE

    def test_clear_verdict_cache_empty_ips_is_noop(self, proxy):
        # No resolved IPs (host never seen / swept) -> nothing to clear.
        proxy.VERDICT_CACHE.clear()
        proxy.VERDICT_CACHE[("10.0.0.1", 1, "9.9.9.9", 443)] = ("allow", 9e9)
        proxy.clear_verdict_cache(set())
        assert proxy.VERDICT_CACHE == {("10.0.0.1", 1, "9.9.9.9", 443): ("allow", 9e9)}

    async def test_dispatch_drop_rule_clears_session_state(
        self, proxy, tmp_path, monkeypatch
    ):
        # End-to-end (#2370): an allow revoke's drop_rule clears the in-session
        # SESSION_HOST_ALLOWS (BEFORE the iptables fork, so a racing SYN can't
        # re-install a fresh ACCEPT during the window) + VERDICT_CACHE (after),
        ...
        proxy.state.SESSION_HOST_ALLOWS.append(
            ("h.test", 443, proxy.EXACT, float("inf"))
        )
        proxy.VERDICT_CACHE[("10.0.0.1", 1, "1.2.3.4", 443)] = ("allow", 9e9)
        c = proxy.SidecarConsentClient("http://h/ev", str(tmp_path / "t"), 5)
        c._connected.set()
    ...
    async def test_dispatch_drop_rule_denied_clears_session_deny(
        ...
        # SESSION_HOST_DENIES BEFORE the iptables fork, so a SYN racing that
        ...
        proxy.state.SESSION_HOST_DENIES.append(
            ("h.test", 443, proxy.EXACT, float("inf"))
        )
        c = proxy.SidecarConsentClient("http://h/ev", str(tmp_path / "t"), 5)
        c._connected.set()
...
class TestSigtermShutdown:
    ...
    async def test_setup_nfq_consumer_returns_nfq_for_unbind(self, proxy, monkeypatch):
        # setup_nfq_consumer returns the bound NFQUEUE so shutdown can unbind
        ...
        assert proxy.setup_nfq_consumer(None) is nfq
    ...
    async def test_setup_nfq_consumer_bind_failure_returns_none(
        ...
        assert proxy.setup_nfq_consumer(None) is None
    ...
    async def test_shutdown_closes_socket_stops_client_unbinds_nfq(self, proxy):
        # shutdown best-effort-closes every resource (#2400); a failure in one
        ...
        await proxy.shutdown(client, nfq, sock, sweep)
    ...
    async def test_sigterm_handler_cancels_main_and_runs_teardown(
        ...
        # must unwind async_main and run shutdown (close the DNS socket).
        ...
        task = asyncio.create_task(proxy.async_main())
    ...
    async def test_sigterm_handler_registered_even_with_consent(
        ...
        monkeypatch.setattr(proxy.app, "resolve_ws_host", lambda url: None)
        ...
        fake_nfq = MagicMock()
        monkeypatch.setattr(
            proxy.nfqueue, "setup_nfq_consumer", lambda client: fake_nfq
        )
        fake_sock = MagicMock()
        fake_sock.close = MagicMock()
        monkeypatch.setattr(proxy.socket, "socket", lambda *a, **k: fake_sock)
    ...
    async def test_second_sigterm_does_not_abort_teardown(self, proxy, monkeypatch):
        # Regression (#2400 review): a second SIGTERM arriving while shutdown
        ...
        monkeypatch.setattr(proxy.nfqueue, "setup_nfq_consumer", lambda client: nfq)
        fake_sock = MagicMock()
        fake_sock.close = MagicMock()
        monkeypatch.setattr(proxy.socket, "socket", lambda *a, **k: fake_sock)
        ...
        for _ in range(100):
            if signal.SIGTERM in handlers:
                break
            await asyncio.sleep(0.01)
        # First SIGTERM -> main task cancels -> shutdown -> client.stop() blocks.
        handlers[signal.SIGTERM]()
        for _ in range(100):
            if in_stop.is_set():
                break
        ...
        # guard this is a no-op; without it the re-cancel aborts shutdown.
        handlers[signal.SIGTERM]()
    ...
    async def test_shutdown_bounds_a_slow_client_stop(self, proxy, monkeypatch):
        # shutdown bounds client.stop() so a stalled WS close handshake (its
        ...
        monkeypatch.setattr(proxy.app, "SHUTDOWN_CLIENT_TIMEOUT", 0.1)
        nfq = MagicMock()
        nfq.get_fd = MagicMock(return_value=11)
        sock = MagicMock()
        ...
        t0 = loop.time()
        await proxy.shutdown(_SlowClient(), nfq, sock, sweep)
    ...
    async def test_shutdown_survives_cancelled_error_from_client_stop(self, proxy):
        # #2657 defense in depth: shutdown's wait_for(client.stop()) guard is
        ...
        await proxy.shutdown(_RaisingClient(), nfq, sock, sweep)
...
# not at all (query_name/nxdomain_for, resolve_ws_host, main). Where an
...
class TestWireHelpers:
    """query_name / nxdomain_for / send_nxdomain (#2834): the wire-level
    ...
    def test_query_name_empty_question(self, proxy, monkeypatch):
        ...
        # decision gate then denies it, fail-closed).
    ...
    def test_send_nxdomain_swallows_sendto_failure(self, proxy, monkeypatch):
        ...
        s = MagicMock()
        s.sendto.side_effect = OSError("client gone")
        proxy.send_nxdomain(s, b"q", ("127.0.0.1", 53))  # must not raise

    def test_send_nxdomain_sends_wire(self, proxy, monkeypatch):
        _FakeQueryName(proxy, monkeypatch)
        s = MagicMock()
        proxy.send_nxdomain(s, b"q", ("127.0.0.1", 53))
        s.sendto.assert_called_once_with(b"WIRE", ("127.0.0.1", 53))


class _FakeUpstreamSock:
    """A socket.socket stand-in for forward_marked: records the MARK
...
class TestForwardMarked:
    """forward_marked / forward_and_learn / forward_and_record (#2834):
    ...
    async def test_success_returns_response_and_closes(self, proxy, monkeypatch):
        ...
        assert await proxy.forward_marked(b"q") == b"resp"
    ...
    async def test_upstream_failure_returns_none(self, proxy, monkeypatch):
        ...
        assert await proxy.forward_marked(b"q") is None
        assert sock.closed  # closed on error too

    async def test_forward_and_learn_none_resp_skips_respond(self, proxy, monkeypatch):
        # Upstream failure: forward_and_learn returns without touching the
        # client-facing socket.
        monkeypatch.setattr(
            proxy.resolve, "forward_marked", AsyncMock(return_value=None)
        )
        respond = AsyncMock()
        monkeypatch.setattr(proxy.resolve, "respond_allowed", respond)
        await proxy.forward_and_learn(MagicMock(), b"q", ("c", 1), "q.name", {443})
        respond.assert_not_awaited()

    async def test_forward_and_learn_resp_answers(self, proxy, monkeypatch):
        monkeypatch.setattr(
            proxy.resolve, "forward_marked", AsyncMock(return_value=b"resp")
        )
        respond = AsyncMock()
        monkeypatch.setattr(proxy.resolve, "respond_allowed", respond)
        s = MagicMock()
        await proxy.forward_and_learn(s, b"q", ("c", 1), "q.name", {443})
    ...
    async def test_forward_and_record_none_resp_skips_respond(self, proxy, monkeypatch):
        ...
        respond = AsyncMock()
        monkeypatch.setattr(proxy.resolve, "respond_recorded", respond)
        await proxy.forward_and_record(MagicMock(), b"q", ("c", 1), "q.name")
        respond.assert_not_awaited()
    ...
    async def test_forward_and_record_resp_answers(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(proxy.resolve, "respond_recorded", respond)
        s = MagicMock()
        await proxy.forward_and_record(s, b"q", ("c", 1), "q.name")
        respond.assert_awaited_once_with(s, b"resp", ("c", 1), "q.name")


class TestRespondEdgePaths:
    """respond_allowed / respond_recorded edge outcomes (#2834): a
    ...
    async def test_no_a_records_still_sends(self, proxy, monkeypatch):
        ...
        learn = MagicMock()
        monkeypatch.setattr(proxy.rules, "learn_all", learn)
        s = MagicMock()
        await proxy.respond_allowed(s, b"resp", ("c", 1), "q", {443})
    ...
    async def test_recorded_no_a_records_still_sends(self, proxy, monkeypatch):
        monkeypatch.setattr(proxy.resolve, "a_records_with_ttl", lambda wire: [])
        record = MagicMock()
        monkeypatch.setattr(proxy.rules, "record_hosts", record)
        s = MagicMock()
        await proxy.respond_recorded(s, b"resp", ("c", 1), "q")
    ...
    async def test_debug_prints_allow_line(self, proxy, monkeypatch, capsys):
        ...
        monkeypatch.setattr(proxy.rules, "learn_all", lambda *a: None)
        await proxy.respond_allowed(MagicMock(), b"r", ("c", 1), "q.name", {443})
    ...
    async def test_debug_prints_resolve_line(self, proxy, monkeypatch, capsys):
        ...
        monkeypatch.setattr(proxy.rules, "record_hosts", lambda *a: None)
        await proxy.respond_recorded(MagicMock(), b"r", ("c", 1), "q.name")
    ...
    async def test_handle_packet_debug_prints_reject_line(
        ...
        await proxy.handle_packet(MagicMock(), b"q", ("c", 1), None)
...
class TestAppTeardownHelpers:
    ...
    resource's failure so one bad close can't abort the rest of shutdown."""
    ...
    async def test_cancel_task_swallows_task_exception(self, proxy):
        ...
        await proxy.cancel_task(t)  # must not raise
        assert t.done()

    async def test_cancel_task_none_is_noop(self, proxy):
        await proxy.cancel_task(None)  # no task (not started) -> no-op
    ...
    async def test_unbind_nfq_swallows_remove_reader_failure(self, proxy, monkeypatch):
        ...
        nfq = MagicMock()
        proxy.unbind_nfq(nfq)  # sync + must not raise
...
class TestResolveWsHost:
    ...
    def test_resolves_host_to_ip(self, proxy, monkeypatch):
        ...
        assert proxy.resolve_ws_host("http://klangkd:8443/ws/x") == "10.1.2.3"
    ...
    def test_unresolvable_host_returns_none_and_prints(
        ...
        assert proxy.resolve_ws_host("http://klangkd/ws") is None
    ...
    def test_hostless_url_returns_none_and_prints(self, proxy, monkeypatch, capsys):
        ...
        monkeypatch.setattr(proxy.app.socket, "gethostbyname", lambda host: "10.1.2.3")
        assert proxy.resolve_ws_host("not-a-url") is None
        assert "could not resolve the klangkd WS host" in capsys.readouterr().out


class TestAsyncMainLoopPaths:
    """async_main's in-loop paths the SIGTERM tests don't reach (#2834):
    ...
    def _stub_static_startup(self, proxy, monkeypatch, fake_sock):
        ...
        # async_main reaches its recv loop without real kernel access.
        monkeypatch.setattr(proxy.config, "CONSENT_URL", "")
        monkeypatch.setattr(proxy.rules, "check_mark", lambda: None)
        monkeypatch.setattr(proxy.socket, "socket", lambda *a, **k: fake_sock)

    async def test_recv_loop_dispatches_to_handle_packet(self, proxy, monkeypatch):
        # One received datagram is handed to handle_packet, then the loop
        ...
        handled = AsyncMock()
        monkeypatch.setattr(proxy.resolve, "handle_packet", handled)
        task = asyncio.create_task(proxy.async_main())
        for _ in range(100):
            if served.is_set():
                break
    ...
    def test_main_runs_and_returns(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(proxy.app, "async_main", _noop)
    ...
    def test_main_swallows_keyboard_interrupt(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(proxy.app, "async_main", _interrupt)
...
class TestConsentClientRunLoop:
    ...
    async def test_sampler_swallows_tick_failure(self, proxy):
        ...
        task = asyncio.create_task(proxy.activity_sampler(_BoomBump(), get_bytes, 0.01))
...
class TestNfqueueEdgePaths:
    ...
    async def test_drain_swallows_transient_failure(self, proxy):
        ...
        proxy.nfqueue.drain(nfq)  # must not raise
    ...
    def test_classify_non_tcp_uses_destination_granularity(self, proxy):
        ...
        assert proxy.nfqueue.classify_packet(payload) == (
            "",
            0,
            "10.2.3.4",
            53,
    ...
    def test_classify_unparseable_returns_none(self, proxy):
        assert proxy.nfqueue.classify_packet(b"\x00" * 8) is None
    ...
    async def test_expired_cached_verdict_re_prompts(self, proxy, monkeypatch):
        ...
        proxy.state.VERDICT_CACHE.clear()
        proxy.nfqueue.INFLIGHT.clear()
        try:
            ...
            proxy.state.VERDICT_CACHE[flow] = ("deny", time.time() - 1)
            client = MagicMock()
            client.connected = True
            client.request = AsyncMock(return_value=("deny", "once"))
            monkeypatch.setattr(proxy.nfqueue, "host_for", lambda ip: "evil.test")
            monkeypatch.setattr(
                proxy.nfqueue, "session_host_allows_ttl", lambda h, p: None
            )
            monkeypatch.setattr(
                proxy.nfqueue, "session_host_denies_ttl", lambda h, p: None
            )
            monkeypatch.setattr(proxy.nfqueue.rules, "reject", lambda *a: None)
            pkt = MagicMock()
            pkt.get_payload = MagicMock(return_value=payload)
            proxy.nfqueue.cb(pkt, client)
            for _ in range(50):
                if pkt.drop.called:
                    break
                await asyncio.sleep(0.01)
    ...
    async def test_deny_session_verdict_is_remembered(self, proxy):
        ...
        proxy.state.SESSION_HOST_DENIES.clear()
        try:
            proxy.nfqueue.remember_session_verdict("deny", "evil.test", 443, 300.0)
            assert proxy.state.SESSION_HOST_DENIES
            host, port, mode, _exp = proxy.state.SESSION_HOST_DENIES[0]
            assert (host, port, mode) == ("evil.test", 443, proxy.EXACT)
        finally:
            proxy.state.SESSION_HOST_DENIES.clear()
    ...
    async def test_remember_session_verdict_once_adds_nothing(self, proxy):
        ...
        proxy.state.SESSION_HOST_ALLOWS.clear()
        proxy.state.SESSION_HOST_DENIES.clear()
        proxy.nfqueue.remember_session_verdict("allow", "ok.test", 443, None)
        proxy.nfqueue.remember_session_verdict("deny", "evil.test", 443, None)
        assert not proxy.state.SESSION_HOST_ALLOWS
        assert not proxy.state.SESSION_HOST_DENIES

    async def test_remember_session_verdict_non_tcp_adds_nothing(self, proxy):
        proxy.state.SESSION_HOST_ALLOWS.clear()
        try:
            proxy.nfqueue.remember_session_verdict("allow", "ok.test", 0, 300.0)
            assert not proxy.state.SESSION_HOST_ALLOWS
        finally:
            proxy.state.SESSION_HOST_ALLOWS.clear()
    ...
    async def test_apply_verdict_allow_learn_failure_still_accepts(
        ...
        pkt = MagicMock()
        await proxy.nfqueue.apply_verdict(
            pkt,
            ("s", 1, "d", 2),
            "10.2.3.4",
            443,
    ...
    async def test_apply_verdict_deny_reject_failure_still_drops(
        ...
        monkeypatch.setattr(proxy.nfqueue.packets, "send_rst", lambda payload: None)
        monkeypatch.setattr(
            proxy.nfqueue.rules,
            "reject",
    ...
    async def test_decide_and_verdict_bounds_the_cache(self, proxy, monkeypatch):
        ...
        try:
            flow = ("172.16.0.9", 40000, "10.2.3.4", 443)
            for i in range(4100):
                proxy.state.VERDICT_CACHE[("10.0.0.1", i, "d", 443)] = (
                    "deny",
                    time.time() + 60,
                )
            client = MagicMock()
            client.request = AsyncMock(return_value=("deny", "once"))
            monkeypatch.setattr(
                proxy.nfqueue, "remember_session_verdict", lambda *a: None
            )
            monkeypatch.setattr(
                proxy.nfqueue,
                "apply_verdict",
            ...
            pkt = MagicMock()
            await proxy.nfqueue.decide_and_verdict(
                pkt, flow, "10.2.3.4", 443, "evil.test", client
            )
            assert len(proxy.state.VERDICT_CACHE) == 1  # cleared + this verdict
        finally:
            proxy.state.VERDICT_CACHE.clear()
            proxy.nfqueue.INFLIGHT.clear()
    ...
    async def test_deny_session_host_swallows_rst_failure(self, proxy, monkeypatch):
        ...
        try:
            pkt = MagicMock()
            proxy.nfqueue.deny_session_host(
                pkt,
                ("s", 1, "d", 2),
                b"payload",
                "10.2.3.4",
...
class TestRulesEdgePaths:
    ...
    def test_allow_all_ports_clears_stale_rejects(self, proxy, monkeypatch):
        ...
        proxy.rules.REJECTED.clear()
        proxy.rules.LEARNED.clear()
        try:
            monkeypatch.setattr(proxy.rules, "remove_reject", MagicMock())
            monkeypatch.setattr(proxy.rules, "install", lambda ip, port: None)
            proxy.rules.REJECTED[("1.2.3.4", 443, 0)] = time.time() + 60
            proxy.rules.allow("1.2.3.4", None, 60)
            assert not proxy.rules.REJECTED  # superseded reject reaped
        finally:
            proxy.rules.REJECTED.clear()
            proxy.rules.LEARNED.clear()
    ...
    def test_install_reject_skipped_when_rule_exists(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(
            proxy.rules, "reject_rule_exists", lambda ip, port, sport: True
        )
        proxy.rules.install_reject("1.2.3.4", 443)
    ...
    def test_install_skipped_when_rule_exists(self, proxy, monkeypatch):
        ...
        monkeypatch.setattr(proxy.rules, "rule_exists", lambda ip, port: True)
        proxy.rules.install("1.2.3.4", 443)
        assert not runs  # no iptables fork for an existing rule

    def test_drop_for_host_swallows_remove_failure(self, proxy, monkeypatch):
        proxy.rules.LEARNED.clear()
        try:
            proxy.rules.LEARNED["1.2.3.4"] = {
                "expire": time.time() + 60,
            ...
            assert not proxy.rules.LEARNED  # record dropped anyway
        finally:
            proxy.rules.LEARNED.clear()
    ...
    def test_drop_for_host_denied_swallows_reject_remove_failure(
        ...
        try:
            ...
            assert not proxy.rules.REJECTED
        finally:
            proxy.rules.REJECTED.clear()
    ...
    def test_sweep_reject_remove_failure(self, proxy, monkeypatch):
        ...
        try:
            now = time.time()
            proxy.rules.REJECTED[("1.2.3.4", 443, 0)] = now - 1
            monkeypatch.setattr(
                proxy.rules,
                "remove_reject",
                MagicMock(side_effect=RuntimeError("iptables")),
    ...
    async def test_async_sweeper_swallows_sweep_failure(self, proxy, monkeypatch):
        ...
        task = asyncio.create_task(proxy.rules.async_sweeper())
    ...
    def test_fmt_ports_all_when_none_present(self, proxy):
        assert proxy.rules.fmt_ports({None}) == "all"
        assert proxy.rules.fmt_ports({443, None}) == "all"  # None dominates
        assert proxy.rules.fmt_ports({443, 8443}) == "443,8443"
...
class TestAllowlistEntryTtls:
    ...
    def test_entry_ttl_empty_host_is_none(self, proxy):
        assert proxy.session_entry_ttl([], "", 443) is None
    ...
    def test_entry_ttl_skips_just_expired_entry(self, proxy):
        ...
        lst = [("h", 443, proxy.EXACT, -1.0)]
        assert proxy.session_entry_ttl(lst, "h", 443) is None
    ...
    def test_rule_cap_skips_just_expired_allow(self, proxy, monkeypatch):
        ...
        proxy.state.SESSION_HOST_ALLOWS[:] = [("h", 443, proxy.EXACT, 100.5)]
        ...
        try:
            assert proxy.session_allow_rule_cap("h") is None
        finally:
            proxy.state.SESSION_HOST_ALLOWS[:] = []
...
class TestPacketHelpers:
    ...
    def test_ipv4_offsets_rejects_non_ipv4(self, proxy):
        assert proxy.packets.ipv4_offsets(b"\x55" + b"\x00" * 30) is None

    def test_ipv4_offsets_rejects_short_payload(self, proxy):
        assert proxy.packets.ipv4_offsets(b"\x45\x00") is None

    def test_ones_checksum_pads_odd_length(self, proxy):
        # RFC 1071: odd data is zero-padded; 3 bytes behave as 4.
        assert proxy.packets.ones_checksum(b"\x01\x02\x03") == (
            proxy.packets.ones_checksum(b"\x01\x02\x03\x00")
        )
    ...
    def test_check_rst_socket_opens_hdrincl_socket(self, proxy, monkeypatch):
        ...
        try:
            ...
            assert proxy.packets.RST_SOCK is fake
...
class TestBranchArcs:
    ...
    def test_parse_specs_non_digit_port_is_host_part(self, proxy, monkeypatch):
        ...
        assert proxy.parse_specs() == [("example.com:x", None, proxy.EXACT)]
    ...
    def test_parse_specs_empty_after_strip_is_skipped(self, proxy, monkeypatch):
        ...
        assert proxy.parse_specs() == [("ok.com", None, proxy.EXACT)]
    ...
    def test_entry_ttl_keeps_max_across_matching_entries(self, proxy):
        ...
        now = time.time()
        lst = [
            ("h", 443, proxy.EXACT, now + 100),
            ("h", 443, proxy.EXACT, now + 50),
        ]
        assert 99 < proxy.session_entry_ttl(lst, "h", 443) <= 100
    ...
    async def test_fail_fast_no_consent_non_tcp_just_drops(self, proxy, monkeypatch):
        ...
        pkt = MagicMock()
        proxy.nfqueue.fail_fast_no_consent(pkt, b"payload", "10.2.3.4", 0)
        pkt.drop.assert_called_once()

    def test_apply_cached_deny_non_tcp_skips_rst(self, proxy):
        # A cached deny for a non-TCP flow: no RST to forge, just drop.
        pkt = MagicMock()
        proxy.nfqueue.apply_cached_verdict(pkt, ("deny", time.time() + 60), b"p", 0)
        pkt.drop.assert_called_once()
    ...
    def test_remember_session_verdict_unknown_decision_adds_nothing(self, proxy):
        ...
        proxy.state.SESSION_HOST_DENIES.clear()
        try:
            proxy.nfqueue.remember_session_verdict("bogus", "h", 443, 300.0)
            assert not proxy.state.SESSION_HOST_ALLOWS
            assert not proxy.state.SESSION_HOST_DENIES
        finally:
            proxy.state.SESSION_HOST_ALLOWS.clear()
    ...
    def test_ipv4_offsets_rejects_bad_ihl(self, proxy):
        ...
        assert proxy.packets.ipv4_offsets(b"\x41" + b"\x00" * 30) is None
