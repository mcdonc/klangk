...
# ---------------------------------------------------------------------------
# UsersModel: the app_state-owned form, reached via ``app_state.model.users``.
#
...
# still used by the test suite (~238 sites), the ACL permission layer
# (``acl.get_principals`` → ``get_user_group_ids``, converted in #1577),
# the schema migration (``backfill_handles``, reworked in #1578), and a few
# app_state-less module helpers (``mentions_agent`` / ``addresses_other_user``).
#
# The agent-user cache stays a module global for now: the backstops' internal
# call chain (e.g. ``set_user_handle`` → ``_assert_handle_not_agent`` →
# ``agent_handle``) needs a module-level cache, so moving it to instance state
# is coupled to removing *all* users backstops — which is the #1578 bulk
# test-conversion job. The cache methods below touch the same module global,
# so app code (``app_state.model.users.agent_handle()``) and backstop callers
# share one cache in the meantime.
# ---------------------------------------------------------------------------
