...
test.describe("file-viewers/markdown", () => {
  ...
  test("download round-trips the markdown source", async ({
    ...
    try {
      await seedFile(request, workspaceId, "/home/klangk/dl.md", MD, headers);
      await openFilesTab(page);
      await clickFileRow(page, 0);
      ...
      expect(Buffer.from(await dl.body()).toString()).toContain(
        "# Heading One",
      );
    } finally {
      await cleanup();
  ...
  test("close, tab-switch, and leave keep clean state", async ({
    ...
    try {
      await seedFile(request, workspaceId, "/home/klangk/nav.md", MD, headers);
      await openFilesTab(page);
      await clickFileRow(page, 0);
