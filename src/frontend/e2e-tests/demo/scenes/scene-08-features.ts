 ...
 * the boingball feature's bouncing-ball animation, hold on the animation. The
 * boingball feature is ALWAYS baked into klangk (every workspace image ships
 * with /opt/klangk/features/boingball + its pi extension at
...
const PROMPT =
  "use the boingball feature to show me a bouncing ball animation, please";

test("features", async ({ page, request }) => {
  ...
  //      (the feature-vs-sandbox trade-off). ---
  await pace(20_000);
});
