...
3. **No per-container LLM config**: The backend injects `KLANGK_LLM_PROXY_URL=http://host.containers.internal:<nginx_port>/llm-proxy` into each container. `klangk-setup-clankers` writes Pi's `models.json` with the proxy URL and `!klangk-workspace-token` as the API key (Pi resolves this command at request time; nginx validates the workspace JWT via `auth_request` before replacing it with the real API key). `KLANGK_LLM_BASE_URL` is only used by nginx itself.
