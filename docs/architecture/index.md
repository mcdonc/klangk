...
- Workspace files: `workspaces/<user-id>/home/<workspace-id>/` (the `/home` bind mount; each member's files live under `.users/<user-id>/`, the shared home under `klangk/`)
- Persistent home: `workspaces/<user-id>/home/<workspace-id>/` (mounted as `/home` — dotfiles, bash history, Pi sessions)
