...
| `manage-events`          | `/events`           | Read every audit stream: the time-correlated merge of all three tables (`GET /events`, #3251), container start/stop (`GET /events/containers`) and the identity/privilege stream (`GET /events/audit`, #3205) — read-only audit |
