...
Async hooks are also supported (`async def`). The hook script can import from `klangk` (e.g. `from klangk.model import get_db`) since the backend packages are on `sys.path` at runtime.
