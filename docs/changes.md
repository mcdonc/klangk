...
- **`KLANGKD_AUDIT_FAIL_CLOSED` (#3154).** Opt-in fail-closed
  ...
  container transitions (POST start/stop/restart, create's eager start,
  delete's stop) write their `container_events` audit row _before_
  acting and refuse the request with a 503 when it cannot be written.
  Autonomous stops (idle timeout, eviction, drain, shutdown sweep,
  crash teardown, boot reaps) are never gated. Default off keeps the
  best-effort behavior. `/health` now also reports
  `audit.write_failures` (a counter bumped on every audit-write
  failure, best-effort paths included) and `audit.fail_closed` so
  assessors can verify the mode. Reloadable on SIGHUP.
