...
- **Native YAML booleans for string-typed boolean settings (#2796).**
  ...
  `allow_insecure_no_auth`, and `reject_proxy_headers` now accept a bare
  `true`/`false` in the YAML config file (previously rejected at boot unless
  quoted). Env vars and quoted strings behave exactly as before.
...
- **Boot auto-start no longer ignores a disabled `allow_autostart`
  ...
  (including `/api/v1/config`) reported auto-start off. The boot gate now
  parses the value the same way.
