...
Every workspace has an AI agent named **klangk** that can answer questions
...
```text
@klangk what files are in the home directory?
```
...
in the chat panel. On each `@klangk` mention it receives, at most, a
...
After an @klangk mention, your subsequent messages automatically route to
...
Without these, the agent is unavailable and @klangk mentions are ignored.

### Agent identity + enabling

The chat agent is **opt-in** (#1977): its `pi --mode rpc` subprocess
...
normal state (the chat surface is visible, but `@klangk` mentions are ignored
...
The agent's identity is **fixed** (#2718): handle `klangk`, email
`klangk@example.com` — the agent *is* the klangk user, matching the
container's UNIX username and `/home/klangk`. The handle and email are
not configurable (the former `KLANGKWS_FEATURE_CHAT_AGENT_HANDLE`/`EMAIL`
keys are removed — stale settings are ignored) and cannot be changed by
an admin. The identity is seeded into the database on startup (and
reconciled on SIGHUP). The agent user cannot have a password and cannot
...
special-case gate. (Manually typing a full-handle `@klangk` would still
...
- **Agent messages** — sent by the agent, shown with a robot icon in cyan
