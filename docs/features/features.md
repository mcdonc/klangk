...
| ------------ | --------------------------------------------------------------------------------------------------------------- | --------------------------------------------- |
| `chat`       | Workspace chat surface + opt-in clanker agent ([#1976](https://github.com/mcdonc/klangk/issues/1976))           | add `chat` to `KLANGKD_FEATURES_ENABLE`       |
...
The workspace chat surface + clanker AI agent, extracted into a compiled-in
...
enabling `chat` shows the tab but does **not** spawn the clanker agent. The
...
`@clanker` mentions are ignored until the agent is enabled. See [Chat](chat.md)
