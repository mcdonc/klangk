...
- **Increase E2E parallelism**: Tests run with `fullyParallel: true` and 4 workers (down from 6.8 min to ~1.9 min). Port allocation race fixed with asyncio lock. At 8+ workers, terminal-based tests fail because the terminal isn't interactive yet when `terminalType` starts typing.
